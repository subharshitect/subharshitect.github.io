(() => {
  ...
  function resizeGrid() {
    ...
    state.cols = U.clamp(Math.floor(w / (exp ? 8.2 : 12.2)), 40, exp ? 144 : 96);
  ...
  function stepOrganism() {
    const seed = U.hash(`${Math.round(knob.control * 1000)}-${Math.round(knob.rigor * 1000)}-${state.frame}`);
    const rand = U.seeded(seed);
    const controlBias = knob.control * 0.34;
    const rigorBias = knob.rigor * 0.28;
    const romance = 1 - knob.rigor;
    const surrender = 1 - knob.control;
    ...
    for (let y = 0; y < state.rows; y += 1) {
      for (let x = 0; x < state.cols; x += 1) {
        ...
        if (knob.control > 0.68 && x > state.cols / 2) {
  ...
  function organismText() {
    ...
    for (let y = 0; y < state.rows; y += 1) {
      ...
      for (let x = 0; x < state.cols; x += 1) {
        ...
        const noisy = U.clamp(v + lesion * (0.08 + (1 - knob.control) * 0.14), 0, 1);
        row += palette[Math.floor(noisy * (palette.length - 1))] || " ";
      }
      lines.push(row);
    }
    return { text: lines.join("\n"), entropy: entropy / (state.cols * state.rows) };
  }
  ...
  function contaminatePortrait(baseLines) {
    ...
    const rr = U.seeded(U.hash(`${state.frame}:${Math.round(knob.signal * 1000)}`));
    const breathe = Math.sin(state.frame * 0.035) * 0.08;
    const coherence = 0.3 + knob.control * 0.6;
    const sharp = 0.2 + knob.rigor * 0.7;
    const smear = (1 - knob.rigor) * 0.22;
    const dissolve = (1 - knob.control) * 0.22;
    const out = [];
